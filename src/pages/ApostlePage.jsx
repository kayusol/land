...
const MINING_ABI=[{type:'function',name:'apostleOnLand',inputs:[{name:'id',type:'uint256'}],outputs:[{type:'uint256'}],stateMutability:'view'}]
...
// 使徒图片：成体 or 孵化状态
// 原版成体图片来自后端API（已下线），我们用不同颜色的孵化器区分
const GH = 'https://raw.githubusercontent.com/evolutionlandorg/evo-frontend/main/public/images'
function apoImg(isAdult, elem, gender) {
  // 成体时用元素颜色的特殊样式（filter hue-rotate）
  return APO_EGG_GIF
}
...
function ApostleCard({apo, isMe, onBreed, onDetail}) {
  ...
  const inMining = apo.landId > 0n
  const progress = apo.growth || 0

  return (
    <div className="ap-card" onClick={()=>onDetail&&onDetail(apo)}
      style={{borderColor: adult ? GENDER_COLOR[apo.gender]+'44' : '#3a2a50'}}>
      {/* 图片区 */}
      <div className="ap-card-img-wrap" style={{background:`linear-gradient(135deg,${ELEMS[apo.element].color}22,#0a0814)`}}>
        ...
        {/* 成长进度条 */}
        {!adult && (
          <div className="ap-growth-bar">
            <div style={{width:`${progress}%`,background:'#c090ff',height:'100%',borderRadius:2}}/>
          </div>
        )}
        {/* 状态badge */}
...
function BreedModal({myApo, onClose, apostles, pc, address, wc, onDone}) {
  const [step, setStep] = useState(1) // 1=选配对方 2=确认
  ...
  async function doBreed() {
    ...
    const maleId   = isMale ? myApo.id : partner.id
    const femaleId = isMale ? partner.id : myApo.id
    setMsg('检查授权...')
    try{
      // approve RING
      if(breedFee > 0n) {
        ...
        {!partner ? (
          <>
            <div style={{padding:'.6rem 1rem .3rem',fontSize:'.78rem',color:'#9070d0'}}>
              选择配对的{isMale?'雌':'雄'}使徒（已成年、无冷却）：
            </div>
            {candidates.length===0
              ? <div style={{padding:'1.5rem',textAlign:'center',color:'#5040a0'}}>
                  暂无可配对的{isMale?'雌':'雄'}使徒
                  <br/><span style={{fontSize:'.72rem'}}>需要已成年（孵化7天后）且无冷却期</span>
                </div>
...
export default function ApostlePage() {
  ...
  const [filter,   setFilter]   = useState('all') // all|mine|male|female|egg|adult
  ...
  const filtered = apostles.filter(a=>{
    if(filter==='mine')   return a.owner?.toLowerCase()===address?.toLowerCase()
  ...
  // 代数分组统计
  const genStats = {}
  apostles.forEach(a=>{ genStats[a.gen]=(genStats[a.gen]||0)+1 })
    ...
    <div className="ap-root">
      <div className="ap-header">
        ...
        <div className="ap-stats">
          {Object.entries(genStats).sort(([a],[b])=>Number(a)-Number(b)).map(([gen,cnt])=>(
            <span key={gen} className="ap-stat-tag">G{gen}: {cnt}</span>
          ))}
          <span className="ap-stat-tag" style={{color:'#52c462'}}>
            成体: {apostles.filter(a=>a.isAdult).length}
          </span>
          <span className="ap-stat-tag" style={{color:'#c090ff'}}>
            孵化中: {apostles.filter(a=>!a.isAdult).length}
          </span>
        </div>
      </div>
      ...
      <div className="ap-info-card">
        <div className="ap-info-row">
          ...
          <span>💕 雄♂+雌♀可以繁殖</span>
          <span>→</span>
          <span>🧬 后代代数+1，继承基因</span>
        </div>
      </div>

      {/* 筛选 */}
      <div className="ap-filters">
        {[['all','全部'],['mine','我的'],['adult','已成体'],['egg','孵化中'],['male','♂ 雄'],['female','♀ 雌']].map(([v,l])=>(
      ...
      {loading
        ...
        : filtered.length===0
          ? <div className="ap-empty">暂无使徒</div>
          : <div className="ap-grid">
              {filtered.map(a=>(
                <ApostleCard key={a.id} apo={a} isMe={a.isMe}
                  onBreed={()=>setBreedApo(a)}
                  onDetail={setDetailApo}
                />
              ))}
            </div>
      }
      ...
      {breedApo && (
        <BreedModal
          myApo={breedApo}
          apostles={apostles}
          onClose={()=>setBreedApo(null)}
          pc={pc} address={address} wc={wc}
          onDone={load}
        />
      )}
    </div>
